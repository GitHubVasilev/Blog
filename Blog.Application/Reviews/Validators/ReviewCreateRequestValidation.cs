...
public class ReviewCreateRequestValidation : AbstractValidator<ReviewCreateRequest>
{
    public ReviewCreateRequestValidation()
    {
        RuleFor(m => m.User).NotEmpty().NotNull();
        RuleFor(m => m.ViewModel.Content).NotEmpty().NotNull().MaximumLength(2048);
        RuleFor(m => m.ViewModel.EntityId).NotEmpty().NotNull();
        RuleFor(m => m.ViewModel.Id).NotEmpty().NotNull();
